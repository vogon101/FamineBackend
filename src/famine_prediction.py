...
def predict_famine(fit, predicted_data):
    ...
    for (i, time) in enumerate(times):
        ...
        if('P2' not in predicted_data[time]):
            ...
            P2_mean = scipy.special.expit(al_2 + be_2[0]*scipy.special.logit(prev_P2) + be_2[1]*scipy.special.logit(prev_P3) + be_2[2]*scipy.special.logit(prev_P4) + sum(np.multiply(co_2, features)))
            P3_mean = scipy.special.expit(al_3 + be_3[0]*scipy.special.logit(prev_P2) + be_3[1]*scipy.special.logit(prev_P3) + be_3[2]*scipy.special.logit(prev_P4) + sum(np.multiply(co_3, features)))
            P4_mean = scipy.special.expit(al_4 + be_4[0]*scipy.special.logit(prev_P2) + be_4[1]*scipy.special.logit(prev_P3) + be_4[2]*scipy.special.logit(prev_P4) + sum(np.multiply(co_4, features)))
